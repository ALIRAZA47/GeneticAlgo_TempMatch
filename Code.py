...
def CrossoverAndMutate(sortedPopulationList, sortDect):
    ...
    while( loopi < populationSize):
        # print(i)
        print(f" ------{loopi}")        
        pt1 = sortedPopulationList[loopi][0]
        pt2 = sortedPopulationList[loopi][1]
        pt3 = sortedPopulationList[loopi+1][0]
        pt4 = sortedPopulationList[loopi+1][1]
        pt1Bin = DecToBin(pt1)
        pt2Bin = DecToBin(pt2)
        pt3Bin = DecToBin(pt3)
        pt4Bin = DecToBin(pt4)
    ...
    # Now Mutation
        ...
        if x1+len(targetImage)<=len(groupImage) and y1+len(targetImage[0]) <= len(groupImage[0]):
            croppedIMG = groupImage[x1:x1+35, y1:y1+29]
            # print("targetImage ---> " )
            # print(len(targetImage), len(targetImage[0]))
            # print(len(croppedIMG), len(croppedIMG[0]))
            newFitVal = match_template(targetImage, croppedIMG)
            p1FitVal = float(newFitVal[0][0][0])
            parent1 = (pt1, pt2)   
            xx = corellatedValues[parent1]
            if (p1FitVal > float(xx)):
                loopi += 1
                populationList.append(temp1)
                # print("done1")
            croppedIMG = groupImage[x2:x2+35, y2:y2+29]
            newFitVal = match_template(targetImage, croppedIMG)
